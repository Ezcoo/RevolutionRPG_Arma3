...
daylight_fnc_shopSpawnMerchants = {
...

	{
		...
		_untMerchant setVariable ["daylight_arrInitialPos", (_x select 4)];
		...
		_untMerchant addEventHandler ["HandleDamage", {
			...
			if (((_this select 0) distance _arrPos) > 1) then {
		...
		[_untMerchant, (_x select 0)] spawn {
			sleep 2.5;
...
daylight_fnc_shopSpawnVehicleShops = {
...

	{
		...
		_untMerchant setVariable ["daylight_arrInitialPos", (_x select 5)];
	...
	if (!dialog) then {
		...
		if ((side player) in _arrSides) then {
			...
			for "_i" from 0 to ((count _arrSoldVehicles) - 1) do {
				...
				_strCar = format ["%1 (%2%3)", _arrCur select 0, round((_arrCur select 4) * (1 + daylight_iPresidentTax)), localize "STR_CURRENCY"];
...
daylight_fnc_shopVehicleBuy = {
	...
	_iPrice = round((_arrCur select 4) * (1 + daylight_iPresidentTax));
	...
	if (count _arrNearVehicles > 0) then {
		...
		if (_bHasMoney) then {
			...
			clearBackpackCargoGlobal _vehVehicle;
