...
daylight_fnc_timeAndWeatherSyncServer = {
	...
	while {true} do {
		...
		if (((time - _iNextSkipTime) >= _iNextSkipTime) || ((time - _iNextSkipTime) >= 0) || (_iNextSkipTime == -1)) then {
			...
			_iNextSkipTime = time + ((_arrDateCurrent select 1) * 3600); // 3600
...
daylight_fnc_timeAndWeatherSyncClient = {
	...
	waitUntil {!(isNil "daylight_arrDate") && !(isNil "daylight_iFog")};
	setDate daylight_arrDate;
