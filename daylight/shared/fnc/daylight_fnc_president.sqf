			...
			if (!(isNull _vehUnit) && ((side _vehUnit) == civilian)) then {
				...
				for "_i" from 0 to (daylight_cfg_iPresidentRulingTime / 5) do {
					...
					if (!(isNull _vehPresident)) then {
						_iBounty = [_vehPresident, format["iBounty%1", _vehPresident call daylight_fnc_returnSideStringForSavedVariables], 0] call daylight_fnc_loadVar;
						
						_iJailTime = [_vehPresident, format["iJailTime%1", _vehPresident call daylight_fnc_returnSideStringForSavedVariables], 0] call daylight_fnc_loadVar;

						_iTimesNull = 0;

						_iJailed = (_vehPresident getVariable "daylight_arrState") select 2;
					} else {
						_iTimesNull = _iTimesNull + 1;
					};

					// Jailtime too long, vote for new president
					if ((_iJailTime > 10) && (_iJailed == 1)) exitWith {};

					// Away for too long, vote for new president
					if (_iTimesNull == 180) exitWith {};

					if ((side _vehUnit) != civilian) exitWith {};
...
daylight_fnc_presidentSpawnVoteUnit = {
...

	{
		...
		[_untVoteUnit, (_x select 0)] spawn {
			sleep 2.5;
...
daylight_fnc_presidentAddVoteOpenUI = {
	if (!dialog && ((side player) == civilian) /*&& ((daylight_arrPresidentInfo select 0) == "")*/) then {
		...
		if ((side player) != civilian) then {
			[localize "STR_PRESIDENTVOTING_TITLE", localize "STR_PRESIDENTVOTING_WRONGSIDE", true] call daylight_fnc_showHint;
		};
	};
};
