...
daylight_fnc_jailJailPlayer = {
	...
	format["Officer %1 jailed %2 for %3 minutes.", name player, name (_this select 0), _this select 1] call daylight_fnc_networkChatNotification;
...
daylight_fnc_jailJailedMonitor = {
	...
	daylight_iSurrenderWeaponsPlaceholderCurrentMagazineAmmo = "";

	[player, false] call daylight_fnc_networkAllowDamage;
	...
	if (!daylight_bJailed) then {
		...
		[player, true] call daylight_fnc_networkAllowDamage;
		...
		if (_bEscaped) then {
			...
			format["%1 was released from jail.", name player] call daylight_fnc_networkChatNotification;
		};
	};
...
daylight_fnc_jailSetUnwanted = {
	...
	// Update bounty
	if (((_arrWanted select (_this select 1)) select 1) > 0) then {
		//[(_this select 0), 0] call daylight_fnc_jailHandleBounty;

		_iBounty = [(_this select 0), format["iBounty%1", (_this select 0) call daylight_fnc_returnSideStringForSavedVariables], 0] call daylight_fnc_loadVar;
		_iBounty = _iBounty - ((_arrWanted select (_this select 1)) select 1);

		[(_this select 0), format["iBounty%1", (_this select 0) call daylight_fnc_returnSideStringForSavedVariables], _iBounty] call daylight_fnc_saveVar;
	};
...
daylight_fnc_jailShareMoneyBLUFOR = {
	_iAmountToSaveOriginal = (_this) / ({side _x == blufor} count playableUnits);

	if (_iAmountToSaveOriginal > 0) then {
		{
			if ((side _x) == blufor) then {
				...
				_iAmountToSave = _iMoneyBank + _iAmountToSaveOriginal;
		...
		[format["Police officers were able to fight against crime and have been awarded %1%2.", _iAmountToSaveOriginal, localize "STR_CURRENCY"], [blufor]] call daylight_fnc_networkChatNotification;
	};
};
