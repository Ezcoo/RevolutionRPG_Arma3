...
daylight_arrPVEndMissionLocationDestroyed = [];
//daylight_arrPVSetGroupID = [];
...
"daylight_arrPVChatNotification" addPublicVariableEventHandler {
	_arrSides = (_this select 1) select 1;
	_bShow = false;

	if ((count _arrSides) == 0) then {
		_bShow = true;
	} else {
	...
	if (_bShow) then {
		systemChat format["** %1", (_this select 1) select 0];
	};
...
"daylight_arrPresidentInfo" addPublicVariableEventHandler {
	...
	if ((name player) == _strName) then {
		systemChat "** You are now the president of Altis, you can use the president menu at the town hall!";

		// Add uniform (ready for 1.22)
		/*daylight_bIsPresident = true;
		daylight_strPresidentOldPreUniform = uniform player;
		...
		// Reset uniform from old president (ready for 1.22)
		/*if (daylight_bIsPresident) then {
			player removeUniform U_NikosAgedBody;
			player addUniform daylight_strPresidentOldPreUniform;

			daylight_bIsPresident = false;
			daylight_strPresidentOldPreUniform = "";
		};*/
	};
...
"daylight_arrCustomLaws" addPublicVariableEventHandler {
	if ((daylight_arrPresidentInfo select 0) != "") then {
...
"daylight_arrPVEndMissionLocationDestroyed" addPublicVariableEventHandler {
	daylight_iLocationsDestroyed = ((_this select 1) select 1);

	if (((_this select 1) select 1) == 0) then {
		if ((side player) == civilian) then {
			systemChat format["** %1 has been destroyed, the revolution has begun!", (_this select 1) select 0];
		} else {
			systemChat format["** %1 has been destroyed, the revolution has begun! Stop the rebels from completing the revolution!", (_this select 1) select 0];
		};
	} else {
		if ((side player) == civilian) then {
			systemChat format["** %1 has been destroyed!", (_this select 1) select 0];
		} else {
			systemChat format["** %1 has been destroyed! Stop the rebels from completing the revolution before it's too late!", (_this select 1) select 0];
		};
	};

	((_this select 1) select 2) spawn daylight_fnc_revolutionExplosionFX;

	if (true) exitWith {};
};

/*"daylight_arrPVSetGroupID" addPublicVariableEventHandler {
	((_this select 1) select 0) setGroupId [((_this select 1) select 1)];
...
daylight_fnc_networkChatNotification = {
	...
	daylight_arrPVChatNotification = _anyOutput;
...
daylight_fnc_networkFreezePlayerToggle = {
	...
	ctrlSetFocus (uiNamespace getVariable "daylight_dsplActive" displayCtrl 1700);
};
...
/*
	Description:	End-mission location destroyed
	Args:			nothing
	Return:			nothing
*/
daylight_fnc_networkEndMissionLocationDestroyed = {
	daylight_arrPVEndMissionLocationDestroyed = _this;
	publicVariable "daylight_arrPVEndMissionLocationDestroyed";

	if (!isDedicated) then {
		if ((_this select 1) == 0) then {
			if ((side player) == civilian) then {
				systemChat format["** %1 has been destroyed, the revolution has begun!", _this select 0];
			} else {
				systemChat format["** %1 has been destroyed, the revolution has begun! Stop the rebels from completing the revolution!", _this select 0];
			};
		} else {
			if ((side player) == civilian) then {
				systemChat format["** %1 has been destroyed!", _this select 0];
			} else {
				systemChat format["** %1 has been destroyed! Stop the rebels from completing the revolution before it's too late!", _this select 0];
			};
		};

		(_this select 2) spawn daylight_fnc_revolutionExplosionFX;
	};
};
...
/*
	...
	Return:			nothing
*/
/*daylight_fnc_networkSetGroupID = {
	daylight_arrPVSetGroupID = _this;
	publicVariable "daylight_arrPVSetGroupID";
