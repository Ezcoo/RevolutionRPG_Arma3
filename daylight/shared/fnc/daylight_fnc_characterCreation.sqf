...
daylight_fnc_characterCreationInit = {
	...
	waitUntil {!(isNil "daylight_arrGangs")};
	waitUntil {!(isNil "daylight_fnc_loadVar")};
	waitUntil {!(isNil "daylight_fnc_saveVar")};
	waitUntil {!(isNil "daylight_logSave")};
	...
	if (!(player call daylight_fnc_characterCreationCharacterExists)) then {
		...
		if ((side player) == civilian) then {
			...
			if (_strLastGang != "") then {
				_iOldGang = [daylight_arrGangs, _strLastGang] call daylight_fnc_findVariableInNestedArray;
...
daylight_fnc_characterSetClothing = {
	...
	if (playerSide == civilian) then {
		...
		//player forceAddUniform ((daylight_cfg_arrStartGearBLUFORClothing select 2) select 0);
