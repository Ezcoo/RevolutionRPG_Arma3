...
daylight_fnc_handleKilledPlayer = {
	...
	if (!(isNil "daylight_camStun")) then {
	...
	if (!(isNull _untKiller) && ((name _untKiller) != "Error: No unit")) then {
		if (_untKiller != player) then {
			// Determine if player was killed with a vehicle
			if ((vehicle _untKiller) != _untKiller) then {
				// If there is no driver, select first from crew
				if (isNull(driver (vehicle _untKiller))) then {
					_untKiller = (crew (vehicle _untKiller)) select 0;
				};

				if (!(isNull _untKiller)) then {
					if (side _untKiller == civilian) then {
						...
						if ((side _untKiller) == blufor) then {
							format["Officer %1 killed %2 with a vehicle!", name _untKiller, name player] call daylight_fnc_networkChatNotification;
						};
					};
				};
			} else {
				...
				if (((playerSide) == civilian) && ((side _untKiller) == blufor)) then {
					format["%2 was killed by officer %1!", name _untKiller, name player] call daylight_fnc_networkChatNotification;
				};
			};
		} else {
			// Was player roadkilled
			_arrNearVehicles = nearestObjects [player, ["LandVehicle"], 10];

			if ((count _arrNearVehicles) > 0) then {
				_vehVehicle = _arrNearVehicles select 0;

				// If speed is enough
				if ((speed _vehVehicle) >= 20) then {
					_untKiller = driver _vehVehicle;

					// If there is no driver, select first from crew
					if (isNull _untKiller) then {
						_untKiller = (crew _vehVehicle) select 0;
					};
