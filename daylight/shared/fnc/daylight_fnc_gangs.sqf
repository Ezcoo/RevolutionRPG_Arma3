...
daylight_fnc_gangsInitServer = {
	...
	onPlayerDisconnected "call daylight_fnc_gangsOnPlayerDisconnected";

	call daylight_fnc_gangsAreasInitServer;
};

daylight_fnc_gangsOnPlayerDisconnected = {
	_strUID = _uid;

	if (isNil "daylight_arrGangs") exitWith {};

	// Go trough gangs array
	for "_i" from 0 to ((count daylight_arrGangs) - 1) do {
		_arrCur = daylight_arrGangs select _i;
		_arrMembers = _arrCur select 1;

		for "_i" from 0 to ((count _arrMembers) - 1) do {
			_strMemberUID = _arrMembers select _i;

			if (_strMemberUID == _strUID) then {
				_arrMembers set [_i, -1];
				_arrMembers = _arrMembers - [-1];
			};
		};

		_arrCur set [1, _arrMembers];
		daylight_arrGangs set [_i, _arrCur];
	};
};
...
daylight_fnc_gangsUpdateUI = {
	...
	((uiNamespace getVariable "daylight_dsplActive") displayCtrl 1703) ctrlSetBackgroundColor [0.88, 0, 0, 1];
	...
	for "_i" from 0 to ((count daylight_arrGangs) - 1) do {
		...
		{
			_vehUnit = _x call daylight_fnc_playerUIDtoPlayerObject;

			if (!(isNull _vehUnit)) then {
				_arrUnits set [count _arrUnits, _x];
			};
		} forEach (_arrCur select 1);

		_arrColor = [0, 0.58, 0, 1];
		if (_iLocked == 1) then {
			_arrColor = [0.88, 0, 0, 1];
		};

		_strName = format["%1 (%2 members)", _arrCur select 0, count _arrUnits];

		lbAdd [1500, _strName];

		lbSetColor [1500, _i, _arrColor];
	...
	if (!_bNoGangs) then {
		...
		if (_iLocked == 1) then {
			...
			((uiNamespace getVariable "daylight_dsplActive") displayCtrl 1703) ctrlSetBackgroundColor [0, 0.58, 0, 1];
...
daylight_fnc_gangsJoinGang = {
	...
	//[player, _arrGang select 0] call daylight_fnc_networkSetGroupID;
...
daylight_fnc_gangsLeaveGang = {
	...
	_iCountIngame = 0;
	{
		...
		if (!(isNull _vehUnit)) then {
			_iCountIngame = _iCountIngame + 1;
		};
	} forEach _arrOldPlayers;

	if (_iCountIngame > 0) then {
...
daylight_fnc_gangsCreateOpenUI = {
	...
	[localize "STR_GANGMENU_TITLE", format[localize "STR_GANGMENU_COSTSMONEY", daylight_cfg_iGangCreationCost, localize "STR_CURRENCY"], true] call daylight_fnc_showHint;
...
daylight_fnc_gangsCreate = {
	...
	if (_bHasMoney) then {
		...
		//[player, _this] call daylight_fnc_networkSetGroupID;
...
daylight_fnc_gangsCaptureNotification = {
	...
	if (_strGang == _strNewGang) then {
		if (_iStatus == 0) then {
			...
			format["Gang area %1 was captured by %2!", _iGangArea, _strNewGang] call daylight_fnc_networkChatNotification;
		};
	} else {
