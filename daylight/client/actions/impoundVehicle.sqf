...
[player, format ["%1 started impounding %2.", name player, getText(configFile >> "CfgVehicles" >> typeOf (_this select 0) >> "displayName")]] call daylight_fnc_networkChatNotificationNear;
...
if (!_bMoved) then {
	...
	[player, format ["%1 finished impounding %2.", name player, getText(configFile >> "CfgVehicles" >> typeOf (_this select 0) >> "displayName")]] call daylight_fnc_networkChatNotificationNear;
} else {
	// You moved text
