...
daylight_fnc_stunApplyStun = {
	...
	if (((side _this) == blufor) && ((side player) == civilian)) then {
		if (_iDistance <= daylight_cfg_iStunMaxDistance) then {
			...
			if (daylight_iStunValue >= 10) then {
				if ((count (weapons player) > 0) && ((vehicle player) == player)) then {
				...
				if (daylight_strSurrenderWeaponsPlaceholderCurrentWeapon != "") then {
					_vehWeaponHolder = createVehicle ["GroundWeaponHolder", [0, 0, 0], [], 0, "NONE"];
					_vehWeaponHolder setPosATL (player modelToWorld [0, 1, 0]);

					_vehWeaponHolder addWeaponCargoGlobal [daylight_strSurrenderWeaponsPlaceholderCurrentWeapon, 1];
				};
				...
				if (daylight_strHolsteredWeapon != "") then {
					...
					_vehWeaponHolder addWeaponCargoGlobal [daylight_strHolsteredWeapon, 1];
