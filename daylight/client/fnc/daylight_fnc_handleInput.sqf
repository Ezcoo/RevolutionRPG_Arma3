...
daylight_iLastSirenTime = 0;
daylight_iLastLightsTime = 0;
	...
	switch (true) do {
		...
		case ((_this select 1) == 0x12) : {
			if (!daylight_bJailed && daylight_bHUDReady && !daylight_bActionBusy && !daylight_bSurrendered && !daylight_bRestrained) then {
				if (daylight_bInteractionMenuOpen && dialog) then {
					closeDialog 0;

					daylight_bInteractionMenuOpen = false;
				} else {
		...
		case ((_this select 1) == 0x21) : {
			if ((time - daylight_iLastSirenTime) >= 0.3) then {
				if ((vehicle player) != player) then {
					0 call daylight_fnc_policeVehicleHandleSirenStatus;
				};

				daylight_iLastSirenTime = time;
			};
		};

		// G - Lights on
		case ((_this select 1) == 0x22) : {
			if ((time - daylight_iLastLightsTime) >= 0.3) then {
				if ((vehicle player) != player) then {
					1 call daylight_fnc_policeVehicleHandleSirenStatus;
				};

				daylight_iLastLightsTime = time;
			};
		};
		...
		case ((_this select 1) == 0x14) : {
			...
			if (((vehicle player) != player) && (driver(vehicle(player)) == player)) then {
				...
				if ([player, _vehTarget] call daylight_fnc_hasKeysFor) then {
					// If yes, open trunk
					if ((time - daylight_iTrunkLastOpenTime) >= 0.5) then {
		...
		case ((_this select 1) == 0x0F) : {
			...
			if (!(_this select 2) && daylight_bHUDReady && ((time - daylight_iLastInventoryOpenTime) >= 1.5) && !daylight_bInvDisabled && !visibleMap) then {
		...
		case ((_this select 1) == 0x3B) : {
			...
			if (((side player) == blufor) || ((getPlayerUID player) in _arrAdmins)) then {
		...
		case ((_this select 1) == 0x3C) : {
			...
			if (daylight_bStatusMenuOpen && dialog) then {
				closeDialog 0;

				daylight_bStatusMenuOpen = false;
			} else {
				call daylight_fnc_statusMenuOpenUI;

				daylight_bStatusMenuOpen = true;
			};
		};
		...
		case (([0x02, 0x03, 0x04, 0x05, 0x0E] find (_this select 1)) != -1) : {
			_bDisableDefaultAction = true;
		};
	};
