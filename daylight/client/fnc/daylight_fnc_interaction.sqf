...
daylight_fnc_interactionOpenUI = {
	...
	if (!dialog && !daylight_bRestrained && !daylight_bSurrendered && !daylight_bJailed && ((vehicle player) == player)) then {
		...
		_iBounty = [daylight_vehInteractionCurrentPlayer, format["iBounty%1", daylight_vehInteractionCurrentPlayer call daylight_fnc_returnSideStringForSavedVariables], 0] call daylight_fnc_loadVar;

		ctrlSetText [1000, format["Interact with %1 (Bounty of %2%3)", name daylight_vehInteractionCurrentPlayer, _iBounty, localize "STR_CURRENCY"]];
...
daylight_fnc_interactionUIMonitor = {
	...
	_ctrlButtonJail = ((uiNamespace getVariable "daylight_dsplActive") displayCtrl 1704);
	...
	while {((player distance daylight_vehInteractionCurrentPlayer) < daylight_cfg_iMaxDistanceFromInteractedUnit) && daylight_bDialogInteractionMenuOpen} do {
		waitUntil {!daylight_bNotificationWindowOpen};

		// Disable buttons that we cant use depending on cursorTarget state
		_arrState = daylight_vehInteractionCurrentPlayer getVariable ["daylight_arrState", [0, 0, 0]];
		_bEnable = false;
		...
		_ctrlButtonJail ctrlSetText (format [localize "STR_INTERACTIONMENU_BUTTON_JAILFOR", round(sliderPosition 1900)]);
...
daylight_fnc_interactionRestrainToggle = {
	if (!daylight_bRestrained) then {
		...
		daylight_ppStunBlurEffect ppEffectCommit 5;
		...
		if (!(isNil "daylight_ppStunBlurEffect")) then {
			daylight_ppStunBlurEffect ppEffectAdjust [0];
		...
		if ((count daylight_arrSurrenderWeaponsPlaceholder) > 0) then {
			...
			player setAmmo [daylight_strSurrenderWeaponsPlaceholderCurrentWeapon, daylight_iSurrenderWeaponsPlaceholderCurrentMagazineAmmo];
...
daylight_fnc_interactionSurrenderToggle = {
	if (!daylight_bRestrained && !daylight_bJailed && ((vehicle player) == player) && (daylight_iStunValue < 4)) then {
		if (!daylight_bSurrendered) then {
			...
			daylight_iSurrenderWeaponsPlaceholderCurrentMagazineAmmo = player ammo daylight_strSurrenderWeaponsPlaceholderCurrentWeapon;
...
daylight_fnc_interactionPayTicket = {
	if (_this) then {
		...
		[player, -(daylight_iCurrentTicketAmount)] call daylight_fnc_jailHandleBounty;
...
daylight_fnc_interactionRemoveIllegalItemsLocal = {
	...
	if (_iIllegalItemsCount != 0) then {
		...
		format ["%1 has been caught for posession of illegal items worth of %2%3, %1 is now wanted by the police.", name player, _iTotalProfit, localize "STR_CURRENCY"] call daylight_fnc_networkChatNotification;
	};
};
...
daylight_fnc_handleInteractKey = {
	...
	if ((player distance _this) < daylight_cfg_iMaxDistanceFromInteractedUnit) then {
		...
		if ((daylight_arrMerchants find _this) != -1) exitWith {
			...
			daylight_bInteractionMenuOpen = true;
		};
...
daylight_fnc_interactionVehicleRemoveIllegalItems = {
	...
	// Add bounty
	_arrNearMen = nearestObjects [player, ["Man"], 50];

	{
		_arrNearMen set [count _arrNearMen, _x];
	} forEach (crew _this);
	...
	{
		if ((isPlayer _x) && !_bBreak) then {
			// Has keys
			_arrKeys = [_x, format["arrKeys%1", _x call daylight_fnc_returnSideStringForSavedVariables], []] call daylight_fnc_loadVar;
	...
	if (_iTotalProfit != 0) then {
		[_vehUnit, "Posession of illegal items", round _iTotalProfit] call daylight_fnc_jailSetWanted;

		format ["%1 has been caught for posession of illegal items worth of %2%3, %1 is now wanted by the police.", name _vehUnit, _iTotalProfit, localize "STR_CURRENCY"] call daylight_fnc_networkChatNotification;
	};
...
/*
	Description:	Pull out passangers
	Args:			obj vehicle
	Return:			nothing
*/
daylight_fnc_interactionVehiclePullOut = {
	["Pulling out passangers..", 0.5] call daylight_fnc_progressBarCreate;
	[1, 4] call daylight_fnc_progressBarSetProgress;

	[player, format["Officer %1 is pulling passangers out of %2.", name player, getText(configFile >> "CfgVehicles" >> typeOf _this >> "displayName")]] call daylight_fnc_networkChatNotificationNear;
