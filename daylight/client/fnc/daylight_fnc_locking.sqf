			...
			if ([player, _vehTarget] call daylight_fnc_hasKeysFor) then {
				...
				if ((_vehTarget call daylight_fnc_toggledVehicleLockedNumber) == 0) then {
					...
					if (_vehTarget != (vehicle player)) then {
						if (!(_vehTarget isKindOf "Air") && !(_vehTarget isKindOf "Ship")) then {
...
daylight_fnc_keychainOpenUI = {
	...
	for "_i" from 0 to ((count _arrKeys) - 1) do {
		...
		if ((isNull _vehVehicle) || ((damage _vehVehicle) >= 0.9)) then {
			_arrKeys set [_i, -1];
		};
	};
	...
	for "_i" from 0 to ((count _arrKeys) - 1) do {
		...
		// Get car text
		_strText = format["%1 (%2m)", getText(configFile >> "CfgVehicles" >> typeOf (_arrKeys select _i) >> "displayName"), round(player distance (_arrKeys select _i))];
