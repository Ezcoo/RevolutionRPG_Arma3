...
daylight_fnc_toolUse = {
	if ([([daylight_iToolItemID, 1] call daylight_fnc_invGetItemWeight)] call daylight_fnc_invCheckWeight) then {
		...
		_iAmount = daylight_iToolItemID call daylight_fnc_invItemAmount;
		_iWeight = [daylight_iToolItemID, _iAmount] call daylight_fnc_invGetItemWeight;

		[format["You got 1x %1, you now have %2x %1 (%3kg)", (daylight_iToolItemID call daylight_fnc_invIDToStr) select 0, _iAmount, _iWeight], 1] call daylight_fnc_showActionMsg;
		...
		["You can't collect more of this item, max weight reached.", 1] call daylight_fnc_showActionMsg;
