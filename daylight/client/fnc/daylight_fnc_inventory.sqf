...
daylight_fnc_invOpenUI = {
	...
	if (!dialog && !daylight_bDialogOpen && !daylight_bRespawning && !daylight_bJailed && !daylight_bSurrendered && !daylight_bRestrained && !daylight_bJailed && !daylight_bActionBusy) then {
		...
		if ((lbSize 1500) == 0) then {
			...
			lbSetColor [1500, 0, [0.3, 0.3, 0.3, 1]];

			daylight_bInvNoItems = true;
		};
...
daylight_fnc_dropItemCreateObject = {
	if (!isDedicated) then {
		...
		// If illegal item and blufor
		if (((_this select 3) >= (daylight_cfg_arrInvIllegalIDRange select 0)) && ((_this select 3) <= (daylight_cfg_arrInvIllegalIDRange select 1))) then {
			if ((side player) == blufor) then {
				_strAction = format["<t color=""#75c2e6"">Seize %1x %2</t>", (_this select 4), ((_this select 3) call daylight_fnc_invIDToStr) select 0];

				_vehItem addAction [_strAction, "daylight\client\actions\removeItemPolice.sqf", [(_this select 3), (_this select 4)], 10, true, true, "", "(player distance _target) < 2.5"];
			} else {
				_vehItem addAction [_strAction, "daylight\client\actions\takeItem.sqf", [(_this select 3), (_this select 4)], 10, true, true, "", "!daylight_bPlayerNears && ((player distance _target) < 2.5)"];
			};
		} else {
