...
daylight_fnc_addDefaultEventHandlers = {
	...
	if ((side player) == blufor) then {
		_iRepairActionCount = 0;

		{
			player addAction [format["<t color=""#75c2e6"">Repair %1</t>", (_x select 0)], "daylight\client\actions\repairEndMissionLocation.sqf", [], 10, true, true, "", format["%1 call daylight_fnc_endMissionAction", _iRepairActionCount]];
		
			_iRepairActionCount = _iRepairActionCount + 1;
		} forEach daylight_cfg_arrEndMissionLocations;
	};

	// Add holster actions
	daylight_strHolsteredWeapon = "";
	player addAction ["<t color=""#75c2e6"">Holster handgun</t>", "daylight\client\actions\holsterUnholsterWeapon.sqf", [], 2, true, true, "", "((vehicle player) == player) && ((_this call daylight_fnc_handleHolsterAction) select 0)"];
	player addAction ["<t color=""#75c2e6"">Unholster handgun </t>", "daylight\client\actions\holsterUnholsterWeapon.sqf", [], 2, true, true, "", "((vehicle player) == player) && ((_this call daylight_fnc_handleHolsterAction) select 1)"];
};
...
daylight_fnc_whistle = {
	if (((time - daylight_iLastWhistleTime) > 5) && !(underwater player) && ((vehicle player) == player)) then {
...
daylight_fnc_showNotificationWindow = {
	daylight_bNotificationWindowOpen = true;
...
daylight_fnc_setDefaultColorCorrection = {
	...
	daylight_ppColorCorrection ppEffectAdjust [
		...
		daylight_cfg_arrDefaultColorCorrection select 2,
...
/*
	Description:	Revolution explosive fx
	Args:			arr location
	Return:			nothing
*/
daylight_fnc_revolutionExplosionFX = {
	_iSpeedOfSound = 340.29;
	_iDist = player distance _this;

	//sleep ((_iDist / _iSpeedOfSound) - 2);

	//[daylight_iLocationsDestroyed] spawn BIS_fnc_earthquake;
...
daylight_fnc_endMissionAction = {
	...
	_strMarker = daylight_cfg_arrEndMissionLocationsMarkers select _this;

	if ((player distance (getMarkerPos _strMarker)) <= 5) then {
		_arrNearSmokes = nearestObjects [getMarkerPos _strMarker, ["test_EmptyObjectForSmoke"], 25];
...
daylight_fnc_intro = {
	...
	1500 cutRsc ["GeneralMsgNoBg", "PLAIN", 0.001, false];
	...
	// Set text to intro text
	_cControl ctrlSetStructuredText (parseText (localize "STR_INTRO"));

	_cControl ctrlSetFade 0;
	_cControl ctrlCommit 5;
...
daylight_fnc_hideHUD = {
	{
		// Make sure type is display
		if ((typeName _x) == "DISPLAY") then {
			if ((ctrlIDD _x) == 300) then {
				_dDisplay = _x;

				{
					_ctrlCur = _dDisplay displayCtrl _x;

					_ctrlCur ctrlSetFade 1;
					_ctrlCur ctrlCommit 0;
