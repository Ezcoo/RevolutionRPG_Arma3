		...
		for "_i" from 0 to ((count playableUnits) - 1) do {
			...
			if ((side _objPlayer) == civilian) then {
				if (!(_objPlayer call daylight_fnc_jailPlayerIsWanted)) then {
					...
					lbAdd [2100, format["(WANTED) %1", name _objPlayer]];
				};
...
daylight_fnc_policeMenuSetWanted = {
	...
	format ["Officer %1 set %2 wanted for %3!", name player, name daylight_objCurrentPoliceMenuPlayer, _this] call daylight_fnc_networkChatNotification;
...
daylight_fnc_policeMenuSetUnwanted = {
	...
	format ["Officer %1 unwanted %2 for %3 (%4)!", name player, name daylight_objCurrentPoliceMenuPlayer, _this select 2, (_this select 0)] call daylight_fnc_networkChatNotification;
...
daylight_fnc_policeMenuReleaseFromJail = {
	...
	format ["Officer %1 released %2 from jail (%3)!", name player, name daylight_objCurrentPoliceMenuPlayer, _this] call daylight_fnc_networkChatNotification;
