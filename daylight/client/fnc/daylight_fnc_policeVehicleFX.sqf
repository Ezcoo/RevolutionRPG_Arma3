		...
		if ((_arrSirenStatus select 0) == 1) then {
			...
			if ((time - _iSirenTimeLast) >= _iSirenTime) then {
				...
				if (!(isNull (driver _this))) then {
					// If player is inside vehicle, we need to play the sound louder
					if ((vehicle player) == _this) then {
						playSound [format ["%1_local", ((daylight_cfg_arrPoliceVehicleSFXSamples select _iSiren) select 0)], true];
					} else {
...
daylight_fnc_policeVehicleHandleSirenStatus = {
	...
	if (isNil "daylight_arrPoliceVehiclesAddedFX") exitWith {};
	if ((daylight_arrPoliceVehiclesAddedFX find (vehicle player)) == -1) exitWith {};
	...
	if (_iSide == 0) then {
		...
		if (_this == 0) then {
			if (!((vehicle player) isKindOf "Ship")) then {
				...
				playSound "128919_ecfike_click_2";
			};
		};

		if (_this == 1) then {
			if ((_arrSirenStatus select 1) == 0) then {
				(vehicle player) setVariable ["daylight_arrSirenStatus", [0, 1], true];
			} else {
				(vehicle player) setVariable ["daylight_arrSirenStatus", [0, 0], true];
			};
...
daylight_fnc_policeVehicleAddFXLoop = {
	daylight_arrPoliceVehiclesAddedFX = [];

	while {true} do {
		{
			if (!((vehicle _x) in daylight_arrPoliceVehiclesAddedFX)) then {
				if (((vehicle _x) isKindOf "Car") || ((vehicle _x) isKindOf "Ship")) then {
					_iSide = (vehicle _x) getVariable ["daylight_iVehicleSide", -1];

					if (_iSide == 0) then {
						(vehicle _x) spawn daylight_fnc_policeVehicleFX;

						daylight_arrPoliceVehiclesAddedFX set [count daylight_arrPoliceVehiclesAddedFX, (vehicle _x)];
					};
				};
			};
