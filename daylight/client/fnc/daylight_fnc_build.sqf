...
daylight_fnc_buildItem = {
	...
	{
		if (([getPosASL player select 0, getPosASL player select 1] distance [(getMarkerPos _x) select 0, (getMarkerPos _x) select 1]) < daylight_cfg_iMinDistanceFromMarker) then {
			_bTooClose = true;
		};
	...
	// Disable collisions with all vehicles in the map
	{
		_vehBuilding disableCollisionWith _x;
	} forEach vehicles;
