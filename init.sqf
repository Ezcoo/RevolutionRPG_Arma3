/*
	Description:	The Altians: Revolution
	Version:		1.0.0
...
createMarkerLocal ["respawn_west", daylight_cfg_arrRespawnHoldPos];
...
_vehFlag addAction ["<t color=""#75c2e6"">Capture Government Building</t>", "daylight\client\actions\captureGovBuilding.sqf", [], 10, true, true, "", "((player distance _target) < 5) && daylight_bGovBuildingCaptureable && ((side player) == civilian) && !daylight_bEndMissionComplete"];
...
if (isServer) then {
	[
		[
			// General settings
			60,		// Time between loops (default 5s)
			false,	// Enable log file
			60,		// Write to log interval (default 30s)		
			false	// Enable debug
		],

		[
			// Server settings, -1 to disable		
			150,	// WeaponHolder stay time		
			300,	// Immobile/destroyed vehicle stay time		
			100,	// Dead body stay time		
			900,	// Explosive stay time		
			-1,	// Mine stay time		
			-1,	// Chemlight (& SmokeShell) stay time (class SmokeShell includes chemlights as well)		
	...
	] execVM "daylight\shared\qbt_optimize.sqf";
};

while {!daylight_bWeatherAndTimeSynced} do {sleep 0.1};
